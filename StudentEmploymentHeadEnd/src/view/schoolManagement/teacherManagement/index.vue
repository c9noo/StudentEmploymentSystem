<template>
    <div class="outer">
        ...
        <!-- 身体部分 -->
        <el-card style="margin-top: 10px;height: 750px;">
            <div v-show="screen == 1">
                <el-table :data="teacherList" border>
                    ...
                    <el-table-column label="操作" align="center">
                        <template #="{ row }">
                            ...
                            <el-button type="warning" icon="edit" title="修改教师信息" size="default"
                                @click="updateTeacher(row.id)" style="margin-left: 20px;"></el-button>
                            <el-button type="info" icon="InfoFilled" title="查看信息" size="default"
                                @click="getTeacherDetail(row.id)" style="margin-left: 20px;"></el-button>
                        </template>
                    </el-table-column>
                </el-table>
...
import { reqGetTeacher, reqBatchTeacher, reqAddTeacher, reqUpdateTeacher, reqDeleteTeacher, reqDeleteBatchTeacher, reqUpdateStatus, reqGetTeacherDetail } from '@/api/school'
...
const batchDelete = async () => {
    if (idArry.value.length > 1) {
        await reqDeleteBatchTeacher(idArry).then(() => {
            ElMessage({
                type: 'success',
                message: '删除成功'
            })
...
const deleteTeacher = async (id: string) => {
    await reqDeleteTeacher(id).then(() => {
        ElMessage({
            type: 'success',
            message: '删除成功'
        })
